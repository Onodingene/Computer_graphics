#include <iostream>
#include <vector>
#include <chrono> // to control the timing
#include <SDL.h>
#include <cmath>
...
SDL_Window* window;
float FL = 5; //flag
float DeltaTime; //it smoothens the entire code
float scaledValue = 2; //the value in which it increases in size
...
void render2(std::vector<Point3D> points, std::vector<std::vector<int>> faces)
{
    ...
    float pulsationSpeed = 0.002; // Adjust the speed as needed (lower value for slower pulsation(zooming in and out))
    ...
    time1 = time2; //to contol the smoothness using delta time
}
